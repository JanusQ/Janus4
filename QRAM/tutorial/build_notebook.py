...
| real-device data plots | checked-in processed experiment data | visualize fidelity and mitigation trends |
...
noise demonstration, inspect level-by-level resource trends, and visualize the
processed real-device datasets stored with the repository.
"""
    ),
    code(
        r"""
...
"""
    ...
    md(
        """
## 8. Plot Checked-In Real-Device Data

The Aer example above uses an artificial noise model. This section switches to
processed real-device data that is already stored under `realdata/data/`. The
code below only loads processed artifacts and draws figures; it does not submit
new hardware jobs.

To keep the tutorial environment lightweight, the plots use CSV files and a
plain Python pickle that can be loaded without `qutip`:

| File | What this section uses it for |
| --- | --- |
| `Fig2b_data_pauli_tracking.pkl` | single-qubit teleportation fidelity for six input states |
| `processed_fidelity.csv` | layer-dependent QRAM fidelity sweep and experimental reference points |
| `error_mitigation_pauli_tracking_fidelity.csv` | query fidelity and accepted-data portion before/after selection |
"""
    ),
    code(
        r"""
...
print("Teleportation fidelity summary from processed real-device data")
...
axes[1].set_title("Processed fidelity sweep")
...
axes[2].set_title("Selection improves query fidelity")
...
7. Used repository CSV files to inspect how QRAM resources scale with the
   circuit level.
8. Loaded processed real-device datasets from `realdata/data/` and plotted
