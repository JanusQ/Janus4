...
function getFidelity(n::Int64,classicalData::Vector{Int64},address::Vector{Int64},addressCoefficient::Vector{Float64},noiseTreeArray::Matrix{Float64},shot::Int64)
...
function getFidelityGate(n::Int64,classicalData::Vector{Int64},address::Vector{Int64},addressCoefficient::Vector{Float64},p::Float64,shot::Int64)
    ...
    @sync @distributed for i = 1:shot
        ...
        expectionArray[:,i] = calculate_expected_ones(a)
        fidelityArraySelect[1,i] = fidelityArray[i]
        # fidelityArraySelect[2,i] = judgeZero(a,1,6)
        for j = 1:n
            fidelityArraySelect[j+1,i] = judgeZero(a,1,2^(j+1)-2)
        end
        # fidelityArraySelect[8,i] = judgeZero(a,1,254)
    end
    return sum(fidelityArray)/shot,fidelityArray,sum(expectionArray, dims=2)[:,1]/shot,fidelityArraySelect
end
...
function add_to_hdf5(filename, key, value)
    if isfile(filename)
...
function sampleQram(n::Int64,shots::Int64,classicalData::Vector{Int64},address::Vector{Int64},addressCoefficient::Vector{Float64},noiseTreeArray::Matrix{Float64},filename;savePicture=true,saveH5=true)
    averageFidelity,fidelityArray = getFidelity(n,classicalData,address,addressCoefficient,noiseTreeArray,shots)
    println(averageFidelity)
    fidelityArray = collect(fidelityArray)
    if saveH5
        add_to_hdf5(filename,"$n",fidelityArray)
    end
    fidelityChangeArray = fidelityChange(fidelityArray,shots)
    n_samples = 1:shots
    plot(n_samples, fidelityChangeArray,
    label="$n-layer",          # 图例标签
    ...
    if savePicture
        savefig("timeError/$n-layer_fidelity_vs_sampling_fullAddress_0.00001.pdf")
    end
end
...
function sampleQramGate(n::Int64,shots::Int64,classicalData::Vector{Int64},address::Vector{Int64},addressCoefficient::Vector{Float64},p::Float64,filepath;savePicture=true,saveH5=true)
    averageFidelity,fidelityArray,expectionArray,fidelityArraySelect = getFidelityGate(n,classicalData,address,addressCoefficient,p,shots)
    println("averageFidelity:$averageFidelity")
    mask = fidelityArraySelect[2, :] .== 1
    println(sum(fidelityArraySelect[1, :] .* mask) / sum(mask))
    println(sum(mask))
    fidelityArray = collect(fidelityArray)
    # filename = "$(shots)_shots_fidelity_vs_sampling_fullAddress_$(p)_Fidelity.h5"
    # filename = joinpath(filepath,filename)
    # if saveH5
    #     add_to_hdf5(filename,"$n",fidelityArray)
    # end
    filename = "$(shots)_shots_fidelity_vs_sampling_fullAddress_$(p)_expectionArray.h5"
    filename = joinpath(filepath,filename)
    if saveH5
        add_to_hdf5(filename,"$n",expectionArray)
    end
    filename = "$(shots)_shots_fidelity_vs_sampling_fullAddress_$(p)_FidelitySelect.h5"
    filename = joinpath(filepath,filename)
    if saveH5
        add_to_hdf5(filename,"$n",fidelityArraySelect)
    end
    fidelityChangeArray = fidelityChange(fidelityArray,shots)
    n_samples = 1:shots
    plot(n_samples, fidelityChangeArray,
    ...
    filename = "$(n)_layer_gate_fidelity_vs_sampling_gate_fullAddress_$(p)_$(shots)_shots.png"
    filename = joinpath(filepath,filename)
    if savePicture
        savefig(filename)
    end
...
function sampleQramGateP(n::Int64,shots::Int64,classicalData::Vector{Int64},address::Vector{Int64},addressCoefficient::Vector{Float64},p::Float64,filepath;savePicture=true,saveH5=true)
    ...
    println(sum(mask))
    fidelityArray = collect(fidelityArray)
    filename = "$(shots)_shots_fidelity_vs_sampling_fullAddress_$(n)_Fidelity.h5"
    filename = joinpath(filepath,filename)
    if saveH5
        add_to_hdf5(filename,"$p",fidelityArray)
    end
    filename = "$(shots)_shots_fidelity_vs_sampling_fullAddress_$(n)_expectionArray.h5"
    filename = joinpath(filepath,filename)
    if saveH5
        add_to_hdf5(filename,"$p",expectionArray)
    end
    filename = "$(shots)_shots_fidelity_vs_sampling_fullAddress_$(n)_FidelitySelect.h5"
    filename = joinpath(filepath,filename)
    if saveH5
        add_to_hdf5(filename,"$p",fidelityArraySelect)
    end
    fidelityChangeArray = fidelityChange(fidelityArray,shots)
    n_samples = 1:shots
    ...
    filename = "$(n)_layer_gate_fidelity_vs_sampling_gate_fullAddress_$(p)_$shots.png"
    filename = joinpath(filepath,filename)
    if savePicture
        savefig(filename)
    end
...
for p in [1e-8,5e-8,1e-7,5e-7,1e-6,5e-6,1e-5,5e-5,1e-4,5e-4,1e-3,0.0038,5e-3,1e-2]
    ...
    if !ispath(full_path)
        mkdir(full_path)
        println("文件夹已创建：", full_path)
    else
        println("文件夹已存在：", full_path)
    end
