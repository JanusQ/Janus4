...
import numpy as np
from qram.qramtemplate.fanoutdatacell import Qram,swap_depth,cswap_depth
...
def test_fidelity(i):
    ...
    for config in [config1]:
        ...
        # fidelities['noise']
        print(f"sparse_num:{sparse_num},data:{data},config:{config.decompose_mode},fidelity:{fidelities['fidelity_rho']}")
...
if __name__ == "__main__":
    level = 2
    savedir = f'results/level{level}_fanout/'
    # test_fidelity(0)
    # exit()
