...
class Qram:
    ...
    def layers_router(self, circuit, router_obj, address_index, mid):
        if router_obj.level == 0 and address_index == 0:
            ...
            if router_obj.left_router is not None:
                self.layers_router(circuit, router_obj.left_router, address_index, router_obj.left)
            if router_obj.right_router is not None:
                self.layers_router(circuit, router_obj.right_router, address_index, router_obj.right)

    def reverse_layers_router(self, circuit, router_obj, address_index, mid):
        if address_index != 0:
            if router_obj.level + 1 > address_index:
                return
            if router_obj.right_router is not None:
                self.reverse_layers_router(circuit, router_obj.right_router,address_index, router_obj.right)
            if router_obj.left_router is not None:
                self.reverse_layers_router(circuit, router_obj.left_router, address_index, router_obj.left)
            if router_obj.level + 2 == address_index and address_index == len(self.address_qubits):
                self.router(circuit, router_obj.qreg, mid, router_obj.left_router.data, router_obj.right_router.data)
    ...
    def decompose_circuit(self, circuit):
        if self.load_bus:
            for idx in self.bus_qubits:
                circuit.h(idx)
        incidents = {i:self.bus_qubits[i] for i in range(self.bus_qubits._size)}
        for level in range(len(self.address_qubits)):
            for router in self.routers[level]:
                circuit.cx(self.address_qubits[level], router.qreg)
        circuit.barrier()
        if self.load_bus:
        ...
        for idx in range(incidents_len):
            self.layers_router(circuit, self.routers[0][0],  idx+self.address_qubits._size,incidents[idx])
        circuit.barrier()
        ...
        for idx in range(incidents_len):
            self.reverse_layers_router(circuit, self.routers[0][0],idx+self.address_qubits._size, incidents[idx])
            circuit.barrier()
        
        for level in range(len(self.address_qubits)-1,-1,-1):
            for router in self.routers[level]:
                circuit.cx(self.address_qubits[level], router.qreg)
