...
def test_fidelity(i):
    ...
    precircuit.h(address_qregs[0])
    precircuit.cx(address_qregs[0],address_qregs[1])
    precircuit.save_density_matrix(qubits=address_qregs, label="init", conditional=False) # conditional is set to 
    config0 = Config()
    config1 = Config()
...
if __name__ == "__main__":
    level = 2
    savedir = f'results/level{level}_ghz/'
    # test_fidelity(0)
    # exit()
    if not os.path.exists(f"{savedir}"):
        os.mkdir(f"{savedir}")
    res = ray.get([test_fidelity.remote(i) for i in range(10)])
