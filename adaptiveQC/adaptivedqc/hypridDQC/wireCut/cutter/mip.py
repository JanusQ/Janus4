...
class MIP_Model(object):
    def __init__(
        ...
        self.env = cp.Envr()
        self.model = self.env.createModel("cut_searching")
        # self.model.Param.OutputFlag = 0
        self._add_variables()
    ...
    def _add_variables(self):
        ...
        for i in range(self.num_subcircuit):
            ...
            for j in range(self.n_vertices):
                j_in_i = self.model.addVar(lb=0.0, ub=1.0, vtype=cp.COPT.BINARY)
        ...
        for i in range(self.num_subcircuit):
            ...
            for j in range(self.n_edges):
                v = self.model.addVar(lb=0.0, ub=1.0, vtype=cp.COPT.BINARY)
        ...
        self.num_cuts = self.model.addVar(
            lb=0, ub=self.max_cuts + 0.1, vtype=cp.COPT.INTEGER, name="num_cuts"
        )
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            self.subcircuit_counter[subcircuit]["size"] = self.model.addVar(
                lb=self.n_vertices
                / self.num_subcircuit
                / self.subcircuit_size_imbalance, ## used constraint the subcircuit size
                ub=self.n_vertices
                / self.num_subcircuit
    ...
    def bin_product(self,x,y):
        '''
        func:计算binary变量x与binary变量y的乘积
        para:   x:      vtype = COPT.BINARY
                y:      vtype = COPT.BINARY
                x_name: string
                y_name: string
                model:  定义的模型
        ...
        '''
        w = self.model.addVar(vtype = cp.COPT.BINARY)
        self.model.addConstr(w<=x)
        self.model.addConstr(w<=y)
        self.model.addConstr(w>=x+y-1)
        return w 
    ...
    def _add_constraints(self):
        ...
        for subcircuit in range(self.num_subcircuit):
            ...
            self.model.addConstr(
                cp.quicksum(
                    [
                        self.bin_product(self.edge_var[subcircuit][i]
        ...
        self.model.setObjective(self.num_cuts,sense= cp.COPT.MINIMIZE)
    ...
    def solve(self):
        try:
            self.model.setParam(cp.COPT.Param.TimeLimit, 300.0)
            self.model.setParam(cp.COPT.Param.Threads, 16)
            result= self.model.solve()
            print(result)
        except (cp.CoptError, AttributeError, Exception) as e:
            print("Caught: " + e.message)
        
        if self.model.getAttr(cp.COPT.Attr.HasMipSol):
            print("has solution")
            self.subcircuits = []
            self.optimal = self.model.status == cp.COPT.OPTIMAL
            self.runtime = self.model.getAttr(cp.COPT.Attr.SolvingTime)
            self.node_count = self.model.getAttr(cp.COPT.Attr.NodeCnt)
            self.mip_gap = self.model.getAttr(cp.COPT.Attr.BestGap)
...
def read_circ(circuit):
    ...
    for u, v, _ in dag.edges():
        if hasattr(u, 'op') and hasattr(v, 'op'):
    ...
    for num_subcircuit in num_subcircuits:
        ...
        if feasible:
            ...
            cut_solution = {
                ...
                "counter": counter,
                "subcircuits_num": len(subcircuits),
            }
            break
        elif verbose:
