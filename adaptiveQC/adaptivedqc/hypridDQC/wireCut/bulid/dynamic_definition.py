...
import numpy as np
from ..excute.evaluate_circuit import evaluate_circ
from ..excute.tools import quasi_to_real
from .metrics import MSE

from ..excute.main import get_num_workers
from .graph import GraphContractor
from ..cutter.post_process import get_reconstruction_qubit_order
import os
from multiprocessing import Process
from .merge_prob import merge_prob
...
class DynamicDefinition(object):
    ...
    def merge_states_into_bins(self):
        ...
        for rank in range(num_workers):
            proc = Process(target=merge_prob, args=(self.data_folder, rank, num_workers))
            proc.start()
            procs.append(proc)
        ...
        for rank in range(num_workers):
            ...
            os.remove("%s/rank_%d_merged_entries.pckl" % (self.data_folder, rank))
