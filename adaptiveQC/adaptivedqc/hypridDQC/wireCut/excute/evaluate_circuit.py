...
from .tools import dict_to_array
...
def evaluate_circ(circuit, backend= None,noise_none = False,noise_device = 'ibmq_belem', num_shots = 4000,options=None,way= 'cloud', simulate=True):
    circuit = copy.deepcopy(circuit)
    max_memory_mb = psutil.virtual_memory().total >> 20
    ## equal /2**20 convert to MB
    max_memory_mb = int(max_memory_mb / 4 * 3)
    if way == 'cloud':
        if circuit.num_clbits == 0:
            circuit.measure_all()
        cloud = IBMCloudRun(shots=num_shots)
        if simulate:
            if noise_none:
                return cloud.simulate(circuit)[0]
            else:
                return cloud.simulate(circuit,noise_device=noise_device)[0]
        else:
            return cloud.systemRun(circuit,backendname=noise_device)[0]
    elif way == 'local':
