...
static inline uint64_t pack_qaddress(uint32_t length_words, uint64_t quantum_addr) {
  return ((uint64_t)length_words << 39) | quantum_addr;
}

// .insn r opcode7, func3, func7, rd, rs1, rs2
#define q_run(rs1) {                                                     \
    asm volatile (                                                       \
        ".insn r 0b0001011, 0b110, 4, x0, %0, x0"                        \
        :                                                                \
        : "r" (rs1));                                                    \
  }

#define q_update(rs1, rs2) {                                             \
    asm volatile (                                                       \
        ".insn r 0b0001011, 0b011, 1, x0, %0, %1"                        \
        :                                                                \
        : "r" (rs1), "r" (rs2));                                         \
  }

#define q_set(rs1, rs2) {                                                \
    asm volatile (                                                       \
        ".insn r 0b0001011, 0b011, 0, x0, %0, %1"                        \
        :                                                                \
        : "r" (rs1), "r" (rs2));                                         \
  }
...
#define q_gen() {                                                        \
    asm volatile (                                                       \
        ".insn r 0b0001011, 0b000, 5, x0, x0, x0"                        \
        :                                                                \
        : );                                                             \
  }

#define q_acquire(rd, rs1) \
    ROCC_INSTRUCTION_DS(0, rd, rs1, 6)
